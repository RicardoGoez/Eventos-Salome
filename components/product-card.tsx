...
export function ProductCard({
  ...
  return (
    <Card className="group overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full">
      {/* Image Container */}
      <Link href={`/producto/${producto.id}`} className="relative w-full aspect-[4/3] overflow-hidden bg-gray-100 block">
        {producto.imagen ? (
          <img
            src={producto.imagen}
            alt={producto.nombre}
      ...
      <CardContent className="p-2 sm:p-2.5 md:p-3 lg:p-4 flex flex-col flex-1">
        <Link href={`/producto/${producto.id}`} className="hover:underline">
        ...
        <div className="mt-auto">
          ...
          {producto.disponible ? (
            <div className="flex flex-col gap-1 sm:gap-1.5 md:gap-2">
              ...
              <div className="flex items-center gap-1 sm:gap-1.5 border rounded-md justify-center px-0.5 sm:px-1 py-0.5">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 flex-shrink-0 p-0"
                  onClick={() => setCantidad(Math.max(1, cantidad - 1))}
                  disabled={cantidad <= 1}
                >
                  <Minus className="h-2.5 w-2.5 sm:h-3 sm:w-3 md:h-4 md:w-4" />
                </Button>
                <span className="w-6 sm:w-8 md:w-10 text-center font-medium text-[10px] sm:text-xs md:text-sm lg:text-base">{cantidad}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 flex-shrink-0 p-0"
              ...
              {/* Botón agregar - Compacto y funcional */}
              <Button
            ...
            <Button disabled className="w-full text-[10px] sm:text-xs md:text-sm lg:text-base h-8 sm:h-9 md:h-10 lg:h-11" variant="outline">
              No disponible
            </Button>
          )}
        </div>
